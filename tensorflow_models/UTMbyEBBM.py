...
class BaitBallsAlgorithm(object):
    ...
    def ZORcalculation (self, indivPos, indiv, pop):
        
        inputList = []
        for i in range(pop.size()):
            if ((not(i == indivPos)) and ((self.individualSize-indiv.matchRate(pop.getIndividual(i)))<=self.ZORrange)):
                row = []
                row.append(self.individualSize-indiv.matchRate(pop.getIndividual(i)))
                row.append(pop.getIndividual(i))
                inputList.append(row)
        inputList.sort(key = lambda i: i[0])

        individuals = []
        for i in range(len(inputList)):
            individuals.append(inputList[i][1])
        
        ZOR = Population(len(individuals), False, self.knownData, individuals)
        
        return ZOR
    
    # Zone Of Orientation (ZOO) calculation 
    def ZOOcalculation (self, indivPos, indiv, pop):
        
        inputList = []
        for i in range(pop.size()):
            if ((not(i == indivPos)) and ((self.individualSize-indiv.matchRate(pop.getIndividual(i)))<=self.ZOOrange)):
                row = []
                row.append(self.individualSize-indiv.matchRate(pop.getIndividual(i)))
                row.append(pop.getIndividual(i))
                inputList.append(row)
        ...
        ZOO = Population(len(individuals), False, self.knownData, individuals)
        
        return ZOO
    
    # Zone Of Attraction (ZOA) calculation 
    def ZOAcalculation (self, indivPos, indiv, pop):
        
        inputList = []
        for i in range(pop.size()):
            if ((not(i == indivPos)) and ((self.individualSize-indiv.matchRate(pop.getIndividual(i)))<=self.ZOArange)):
                row = []
                row.append(self.individualSize-indiv.matchRate(pop.getIndividual(i)))
                row.append(pop.getIndividual(i))
                inputList.append(row)
        ...
        ZOA = Population(len(individuals), False, self.knownData, individuals)
        
        return ZOA
    
    # Attraction operator
    def attraction(self, indiv1, indiv2):
        newSol = Individual(self.knownData)
        # Loop through genes
        for i in range(indiv1.size()):
            # Crossover
            if (rnd.uniform(0,1) <= self.attractionRate):
    ...
    def orientation(self, indiv1, indiv2):
        newSol = Individual(self.knownData)
        # Loop through genes
        for i in range(indiv1.size()):
            # Crossover
            if (rnd.uniform(0,1) <= self.orientationRate):
                newSol.setGene(i, indiv1.getGene(i))
            else:
                newSol.setGene(i, indiv2.getGene(i))
        return newSol
    ...
    # Repulsion operator
    def repulsion(self, indiv):
        newSol = Individual(self.knownData)
        # Loop through genes
        for i in range(newSol.size()):
            if (rnd.uniform(0,1) > self.repulsionRate):
                # Leave indiv gene
    ...
    def evolvePopulation(self, pop):
        ...
        #indivStates = ""
        for i in range (pop.size()):
            ZOR = self.ZORcalculation(i,pop.getIndividual(i),pop)
            if (ZOR.size()>=1):
                #indivStates += "R"
                newPopulation.saveIndividual(i,self.repulsion(newPopulation.getIndividual(i)))
            else:
                ZOO = self.ZOOcalculation(i,pop.getIndividual(i),pop)
                if (ZOO.size()>=1):
                    #indivStates += ("O")
                    newPopulation.saveIndividual(i,self.orientation(pop.getIndividual(i),ZOO.getFittest()))
                else:
                    ZOA = self.ZOAcalculation(i,pop.getIndividual(i),pop)
                    if (ZOA.size()>=1):
                        #indivStates += ("A")
                        newPopulation.saveIndividual(i,self.attraction(pop.getIndividual(i),ZOA.getFittest()))
        #print(indivStates)
...
evolution = BaitBallsAlgorithm(knownData)
